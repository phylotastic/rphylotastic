test_that("Get tree from traits works", {
  ...
  result_otol_otol <- traits_get_tree(my_data)
  expect_equal(ape::Ntip(result_otol_otol$phy),10)
  expect_equal(nrow(result_otol_otol$traits),10)
  result_otol_datelife <- traits_get_tree(my_data, tree_source="datelife")
  expect_true(ape::is.ultrametric(result_otol_datelife$phy))
  result_gnr_datelife <- traits_get_tree(my_data, tnrs_source="gnr", tree_source="datelife")
...
test_that("Get tree from data works if someone puts in a vector", {
  ...
  expect_warning(result_otol_otol <- traits_get_tree(my_vector))
  expect_equal(ape::Ntip(result_otol_otol$phy),10)
})
