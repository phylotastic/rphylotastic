...
taxon_names <- c("Asteraceae", "Cephalotus", "Byblis",
          "Drosera", "Sarracenia",
          "Utricularia", "Catopsis berteroniana", "Roridulaceae")
phy1 <- taxa_get_otol_tree(taxa = taxon_names)
phy1
plot(phy1)
# phy2 <- taxa_get_phylomatic_tree(taxa = taxon_names)  # this service is not working for now (2018.06.21)

# phy3 <- taxa_get_taxonomic_tree(taxa = taxon_names) # this service is not working either (2018.06.21); function commented out
...
library(ape)
par(xpd = FALSE, mfrow = c(1, length(trees)), mai = c(0.1, 0.1, 1, 0.1), oma = c(0, 0, 3, 0))
for (i in 1:length(trees)){
  plot(trees[[i]], cex = 0.5, label.offset = 0.1)
  mtext(paste("from", source_tree[i]), cex = 2)
}
mtext("Carnivorous plant tree", outer = T, side = 3, cex = 3)
...
# Managing your lists of species
