...
make_allservices <- function(all_categories){
    ...
    all_services[[all_categories[1]]] <- c("NCBI_common_name",
        ...
        "EOL_common_name")
    all_services[[all_categories[2]]] <- c("GNRD_wrapper_URL; GNRD_wrapper_text; GNRD_wrapper_file",
        "TaxonFinder_wrapper_URL; TaxonFinder_wrapper_text")
    ...
    all_services[[all_categories[5]]] <- c("Image_url_species",
        "Info_url_species",
        "ECOS_Conservation")
    all_services[[all_categories[6]]] <- c("OToL_wrapper_Tree; OToL_supported_studies",
    ...
    all_services[[all_categories[9]]] <- c("Add_new_list; Get_list; Replace_species_list; Update_metadata_list; Remove_list")
...
make_alldescriptions <- function(all_services){
    ...
    all_descriptions[[all_categories[9]]] <- c("Save, publish, access, remove or update lists of names.", "")
