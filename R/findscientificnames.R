...
#' @seealso \url{https://github.com/phylotastic/phylo_services_docs/blob/master/ServiceDescription/PhyloServicesDescription.md}
#' @export
GetScientificNamesFromText <- function(URL, search.engine=0) {
  results <- jsonlite::fromJSON(base::URLencode(paste(GetBaseURL(), 'names_url?url=', URL, '&engine=', search.engine, sep="")))
  return(results$scientificNames)
}
